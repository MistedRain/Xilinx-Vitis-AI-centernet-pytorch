...
void forward_maxpool_layer(int input_w,int input_h,int input_c,int out_h,int out_w,int pad,int ksize,int stride,float *input,float *output)
{
    int i,j,k,m,n;
    int w_offset = -pad;
    int h_offset = -pad;
...
image load_image_cv(const cv::Mat& img,float *mean,float *std_a) {
  int h = img.rows;
  int w = img.cols;
  int c = img.channels();
...
void letterbox_image(cv::Mat img,vector<float>&input_img,int w, int h,int new_w,int new_h){
  // int im_w = img.cols;
  // int im_h = img.rows;
  ...
  input_img.resize(w*h*3);

  unsigned char *data = img.data;

  fill(input_img.begin(),input_img.end(),0.5);
  int dx = (w - new_w)/2;
  int dy = (h - new_h)/2;
  ...
  for(int row = 0;row < new_h ; row++){
    for(int col = 0;col < new_w ; col++){
      for(int c = 0;c < 3;c++){
        float temp = data[row * new_w * 3 + col*3 + c]/256.;
        input_img[(row+dy)*w*3 + (col+dx)*3 + c] = temp;
      }
    }
  }
